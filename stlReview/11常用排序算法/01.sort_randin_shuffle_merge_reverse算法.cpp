...
void test() 
{
	...
	for_each(v.begin(), v.end(), myPrint);// 15 12 11 10
	cout << endl;
...
void test3() 
{
	...
	sort(v3.begin(),v3.end(),greater<int>());//降序排
	for_each(v3.begin(), v3.end(), myPrint);//22 21 20 19 15 14 13 12
	cout << endl;
...
void test4() 
{
	...
	v.push_back(15);
	cout << "Before reverse-----\n";
	for_each(v.begin(), v.end(), myPrint);//12 13 14 15
	
	cout << endl;

	cout << "After reverse------\n";
	reverse(v.begin(), v.end());
	for_each(v.begin(), v.end(), myPrint);//15 14 13 12 
	cout << endl;

}
int main() {
	//添加随机数种子 //random_shuffle(起始迭代器,结束迭代器) 指定范围内的元素随机调正次序才可以真正的随机
